...
#			from kivy.uix.slider import Slider
	...
	# Create the rest of the UI objects (and bind them to callbacks, if necessary):
                highbeam_button = ToggleButton(text="High\nBeam\nLights", halign='center', size_hint=(.14,.23),pos = (2,20), font_size=25)
                highbeam_button.bind(on_press=press_callback)
                lowbeam_button = ToggleButton(text="Driving\nLights", halign='center' , size_hint=(.14,.23),pos = (116,20), font_size=25)
                lowbeam_button.bind(on_press=press_callback)
                leftsig_button = ToggleButton(text="Left\nSignal", group='signals', halign='center' , size_hint=(.14,.23),pos = (230,20), font_size=25)
                leftsig_button.bind(on_press=press_callback)
                hazard_button = ToggleButton(text="Hazard", group='signals',size_hint=(.14,.23),pos = (344,20), font_size=25)
                hazard_button.bind(on_press=press_callback)
                rightsig_button = ToggleButton(text="Right\nSignal", group='signals',halign='center',size_hint=(.14,.23),pos = (458,20), font_size=25)
                rightsig_button.bind(on_press=press_callback)
                wiper_button = ToggleButton(text="Wiper",size_hint=(.14,.23),pos = (572,20), font_size=25)
                wiper_button.bind(on_press=press_callback)
                horn_button = Button(text="BEEP!",size_hint=(.14,.23),pos = (686,20), font_size=25)
